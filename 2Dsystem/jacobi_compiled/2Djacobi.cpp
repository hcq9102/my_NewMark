...
int main()
{
    ...
    */
    ...
    for (int iel = 0; iel < nel; ++iel) { // loop over the total number of elements
        ...
        for (int intx = 0; intx < nglx; intx++) {
            double x = point2(intx,0);
            double wtx = weight2(intx,0);

            for (int inty = 0; inty < ngly; inty++) {
                double y = point2(inty,1);
                double wty = weight2(intx,1);


                // compute shape functions and derivatives at sampling points
                feisoq4(x,y);
                // compute Jacobian
                fejacobi2(nnel,dhdr,dhds,xcoord,ycoord); 

                // determinant of Jacobian
                double detjacob= det(jacobi2);
                //inverse of Jacobian matrix
                blaze::DynamicMatrix<double> invjacob;
                invjacob=inv(jacobi2);
                ...
                -----------------------------------
                */
                kel=kel+ trans(kinmtx2) * matmtx * kinmtx2 * wtx * wty * detjacob;   //element stiffness matrix
            }
        }

        //extract system dofs for the element
        feeldofk(nd,nnel,ndof);                        
   ...
   SOLUTION OF THE WAVEFORM RELAXATION STEPS
    ---------------------------------------------------------------
    */ 
    DynamicVector<double,columnVector> d1p(sdof);
    DynamicVector<double,columnVector> v1p(sdof);
    DynamicVector<double,columnVector> b(sdof);
    ...
    DynamicVector<double,columnVector > a_int(sdof);
